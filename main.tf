...
resource "abbey_grant_kit" "abbey_demo_site" {
  ...
  workflow = {
    steps = [
      {
        reviewers = {
          one_of = ["iliofotou@gmail.com"] # CHANGEME
        }
      }
    ]
  }
  ...
  policies = [
    { bundle = "github://imarios/test-abbey-starter-kit/policies" } # CHANGEME
  ]

  output = {
    # Replace with your own path pointing to where you want your access changes to manifest.
    # Path is an RFC 3986 URI, such as `github://{organization}/{repo}/path/to/file.tf`.
    location = "github://imarios/test-abbey-starter-kit/access.tf" # CHANGEME
    append = <<-EOT
